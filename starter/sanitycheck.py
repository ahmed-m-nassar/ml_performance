...
def run_sanity_check(test_dir):
    ...
    test_functions_for_get = list(filter(lambda x: inspect
                                         .getsource(getattr(module, x))
                                         .find('.get(') != -1,
                                         test_function_names))
    test_functions_for_post = list(filter(lambda x: inspect
                                          .getsource(getattr(module, x))
                                          .find('.post(') != -1,
                                          test_function_names))
    ...
    if not test_functions_for_get:
        ...
        if not TEST_FOR_GET_METHOD_RESPONSE_BODY:
            ...
            print(FAIL_COLOR + """Your test case for GET() does
                                  not seem to be testing the CONTENTS
                                  of the response.\n""")
    ...
    if not test_functions_for_post:
        ...
        print(FAIL_COLOR + """Please make sure you have TWO test cases
                              for the POST() method.\nOne test case for
                              EACH of the possible inferences (results/outputs)
                              of the ML model.\n""")
        SANITY_TEST_PASSING = False
    else:
        if len(test_functions_for_post) == 1:
            ...
            print(FAIL_COLOR + """Please make sure you have two
                                  ...
                                  test case for EACH of the possible inferences
    ...
    print(WARN_COLOR + """This is a heuristic based sanity
                          testing and cannot guarantee the
                          correctness of your code.""")
    print(WARN_COLOR + """You should still check your
                          work against the rubric to
